...
# Removed: discover_printers_tool, print_stl_tool, get_print_status_tool, iterate_cad_tool, generate_cad
...
config = types.LiveConnectConfig(
    ...
    system_instruction="Your name is Lexi, a personal AI assistant. "
        "You have a warm, professional, and helpful personality. "
        "You speak English but understand Swedish. "
        "When the user speaks Swedish, understand them and respond in English voice. "
        "You help with productivity tasks like creating presentations, writing reports, and managing smart home devices. "
...
# NOTE: CadAgent and PrinterAgent removed - CAD/3D functionality disabled
...
class AudioLoop:
                        ...
                        for fc in response.tool_call.function_calls:
                            if fc.name in ["generate_cad", "run_web_agent", "write_file", "read_directory", "read_file", "create_project", "switch_project", "list_projects", "list_smart_devices", "control_light", "discover_printers", "print_stl", "get_print_status", "iterate_cad"]:
                                ...
                                if not confirmation_required:
                                    print(f"[LEXI DEBUG] [TOOL] Permission check: '{fc.name}' -> AUTO-ALLOW")
                                    ...
                                    print(f"[LEXI DEBUG] [STOP] Requesting confirmation for '{fc.name}' (ID: {request_id})")
                                    ...
                                    print(f"[LEXI DEBUG] [CONFIRM] Request {request_id} resolved. Confirmed: {confirmed}")

                                    if not confirmed:
                                        print(f"[LEXI DEBUG] [DENY] Tool call '{fc.name}' denied by user.")
                                ...
                                if fc.name == "generate_cad":
                                    print(f"\n[LEXI DEBUG] --------------------------------------------------")
                                    print(f"[LEXI DEBUG] [TOOL] Tool Call Detected: 'generate_cad'")
                                    print(f"[LEXI DEBUG] [IN] Arguments: prompt='{prompt}'")
                                    ...
                                    print(f"[LEXI DEBUG] [TOOL] Tool Call: 'run_web_agent' with prompt='{prompt}'")
                                    ...
                                    print(f"[LEXI DEBUG] [RESPONSE] Sending function response: {function_response}")
                                    function_responses.append(function_response)
                                    ...
                                    print(f"[LEXI DEBUG] [TOOL] Tool Call: 'write_file' path='{path}'")
                                    ...
                                    path = fc.args["path"]
                                    print(f"[LEXI DEBUG] [TOOL] Tool Call: 'read_directory' path='{path}'")
                                    ...
                                    path = fc.args["path"]
                                    print(f"[LEXI DEBUG] [TOOL] Tool Call: 'read_file' path='{path}'")
                                    ...
                                    name = fc.args["name"]
                                    print(f"[LEXI DEBUG] [TOOL] Tool Call: 'create_project' name='{name}'")
                                    ...
                                    name = fc.args["name"]
                                    print(f"[LEXI DEBUG] [TOOL] Tool Call: 'switch_project' name='{name}'")
                                    ...
                                    if success:
                                        ...
                                        print(f"[LEXI DEBUG] [PROJECT] Sending project context to AI ({len(context)} chars)")
                                        try:
                                            await self.session.send(input=f"System Notification: {msg}\n\n{context}", end_of_turn=False)
                                        except Exception as e:
                                            print(f"[LEXI DEBUG] [ERR] Failed to send project context: {e}")
                                    function_response = types.FunctionResponse(
                                        id=fc.id, name=fc.name, response={"result": msg}
                                    )
                                    function_responses.append(function_response)
                                    ...
                                    print(f"[LEXI DEBUG] [TOOL] Tool Call: 'list_projects'")
                                    ...
                                    print(f"[LEXI DEBUG] [TOOL] Tool Call: 'list_smart_devices'")
                                    ...
                                    print(f"[LEXI DEBUG] [TOOL] Tool Call: 'control_light' Target='{target}' Action='{action}'")
                                    ...
                                    print(f"[LEXI DEBUG] [TOOL] Tool Call: 'discover_printers'")
                                    ...
                                    print(f"[LEXI DEBUG] [TOOL] Tool Call: 'print_stl' STL='{stl_path}' Printer='{printer}'")
                                    ...
                                    printer = fc.args["printer"]
                                    print(f"[LEXI DEBUG] [TOOL] Tool Call: 'get_print_status' Printer='{printer}'")
                                    ...
                                    print(f"[LEXI DEBUG] [TOOL] Tool Call: 'iterate_cad' Prompt='{prompt}'")
                                    ...
                                    if cad_data:
                                        print(f"[LEXI DEBUG] [OK] CadAgent iteration returned data successfully.")
                                        
                                        # Dispatch to frontend
                                        if self.on_cad_data:
                                            print(f"[LEXI DEBUG] [SEND] Dispatching iterated CAD data to frontend...")
                                            self.on_cad_data(cad_data)
                                            print(f"[LEXI DEBUG] [SENT] Dispatch complete.")
                                        ...
                                        print(f"[LEXI DEBUG] [ERR] CadAgent iteration returned None.")
    ...
    async def run(self, start_message=None):
        ...
        while not self.stop_event.is_set():
            try:
                print(f"[LEXI DEBUG] [CONNECT] Connecting to Gemini Live API...")
                async with (
                    ...
                    if not is_reconnect:
                        if start_message:
                            print(f"[LEXI DEBUG] [INFO] Sending start message: {start_message}")
                        ...
                        print(f"[LEXI DEBUG] [RECONNECT] Connection restored.")
                        # Restore Context
                        print(f"[LEXI DEBUG] [RECONNECT] Fetching recent chat history to restore context...")
                ...
                print(f"[LEXI DEBUG] [STOP] Main loop cancelled.")
                break
                
            except Exception as e:
                # This catches the ExceptionGroup from TaskGroup or direct exceptions
                print(f"[LEXI DEBUG] [ERR] Connection Error: {e}")
                
                if self.stop_event.is_set():
                    break
                
                print(f"[LEXI DEBUG] [RETRY] Reconnecting in {retry_delay} seconds...")
