# Lexi V2 - Advanced Design Assistant
...
> **Lexi** = **A**dvanced **D**esign **A**ssistant

Lexi is a sophisticated AI assistant designed for multimodal interaction. It combines Google's Gemini 2.5 Native Audio with computer vision, gesture control, and 3D CAD generation in a Electron desktop application.
...
Lexi's "Minority Report" interface uses your webcam to detect hand gestures:
...
graph TB
    ...
    subgraph Backend ["Backend (Python 3.11 + FastAPI)"]
        ...
        Lexi[ada.py<br/>Gemini Live API]
...
```bash
# 1. Clone and enter
git clone https://github.com/nazirlouis/lexi.git && cd lexi

# 2. Create Python environment (Python 3.11)
conda create -n lexi python=3.11 -y && conda activate lexi
...
# 5. Run!
conda activate lexi && npm run dev
```
...
2. Type this command and hit Enter:
   ```bash
   git clone https://github.com/nazirlouis/lexi.git
   ```
3. This creates a folder named `lexi`.

**Step 5: Open in VS Code**
1. Open VS Code.
2. Go to **File > Open Folder**.
3. Select the `lexi` folder you just downloaded.
...
```bash
conda create -n lexi python=3.11
conda activate lexi
...
To use the secure voice features, Lexi needs to know what you look like.

1. Take a clear photo of your face (or use an existing one).
2. Rename the file to `reference.jpg`.
3. Drag and drop this file into the `lexi/backend` folder.
...
Lexi can slice STL files and send them directly to your 3D printer.
...
Lexi uses **OrcaSlicer** (recommended) or PrusaSlicer to generate G-code.
1. Download and install [OrcaSlicer](https://github.com/SoftFever/OrcaSlicer).
2. Run it once to ensure profiles are created.
3. Lexi automatically detects the installation path.

**Step 2: Connect Printer**
1. Ensure your printer and computer are on the **same Wi-Fi network**.
2. Open the **Printer Window** in Lexi (Cube icon).
3. Lexi automatically scans for printers using mDNS.
...
Lexi uses Google's Gemini API for voice and intelligence. You need a free API key.

1. Go to [Google AI Studio](https://aistudio.google.com/app/apikey).
2. Sign in with your Google account.
3. Click **"Create API Key"** and copy the generated key.
4. Create a file named `.env` in the `lexi` folder (same level as `README.md`).
...
You have two options to run the app. Ensure your `lexi` environment is active!

### Option 1: The "Easy" Way (Single Terminal)
The app is smart enough to start the backend for you.
1. Open your terminal in the `lexi` folder.
2. Activate your environment: `conda activate lexi`
3. Run:
   ```bash
   npm run dev
   ```
...
### 🖨️ Printing & Slicing
- **Auto-Discovery**: Lexi automatically finds printers on your network.
- **Slicing**: Click "Slice & Print" on any generated 3D model.
- **Profiles**: Lexi intelligently selects the correct OrcaSlicer profile based on your printer's name (e.g., "Creality K1").
...
**Solution**:
1. Make sure your `.env` file is in the root `lexi` folder (not inside `backend/`).
